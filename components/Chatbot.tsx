...
export default function Chatbot() {
  ...
  return (
    <div className="fixed bottom-2 right-2 z-[60] sm:bottom-6 sm:right-6">
      {isOpen && (
        <section id="nayilie-chatbot" role="dialog" aria-modal="false" aria-labelledby="chatbot-title" className="mb-2 flex h-[min(520px,70dvh)] w-[calc(100vw-1rem)] max-w-sm flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/25 sm:mb-4 sm:h-[min(560px,calc(100vh-7rem))] sm:w-[calc(100vw-2.5rem)] sm:rounded-[1.5rem]">
          <header className="flex items-center justify-between bg-[#021d47] px-4 py-3 text-white sm:px-5 sm:py-4">
      ...
      <button type="button" onClick={() => setIsOpen((open) => !open)} aria-expanded={isOpen} aria-controls="nayilie-chatbot" className="group flex items-center gap-3 rounded-full bg-[#021d47] p-2 text-sm font-bold text-white shadow-xl shadow-[#021d47]/30 transition hover:-translate-y-0.5 hover:bg-[#032965] focus:outline-none focus:ring-2 focus:ring-[#52a234] focus:ring-offset-2 sm:py-2 sm:pl-2 sm:pr-5">
        <span className="relative flex h-11 w-11 items-center justify-center rounded-full bg-[#52a234] text-[#021d47]"><span className="absolute right-0 top-0 h-3 w-3 rounded-full border-2 border-[#021d47] bg-white" /><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-6 w-6"><path strokeLinecap="round" strokeLinejoin="round" d="M20 11.5a7.5 7.5 0 0 1-8 7.48A8.3 8.3 0 0 1 8.3 18L4 20l1.25-3.55A7.47 7.47 0 1 1 20 11.5Z" /></svg></span><span className="hidden sm:inline">{isOpen ? 'Fermer' : 'Besoin d’aide ?'}</span>
      </button>
    </div>
  )
}
