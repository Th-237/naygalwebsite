      ...
      <section className="container-custom py-14 sm:py-20">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {displayedResources.length ? displayedResources.map((resource) => {
            const cardHref = resource.href && resource.href.endsWith('.pdf')
              ? resource.href
              : 'htmlPath' in resource
                ? `/ressources/${resource.category.toLowerCase()}/${resource.slug}`
                : '/contact'
            ...
            return (
              <article key={resource.title} className="group flex min-h-72 flex-col rounded-3xl bg-white p-7 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="mt-auto pt-6">
                  <Link href={cardHref} className="text-sm font-bold group-hover:text-[#438a2c]">
                    {(() => {
                      const type = resource.type?.toLowerCase()
                      if (type === 'article') return 'Lire l’article →'
                      if (type === 'guide') return 'Ouvrir le guide →'
                      if (type === 'conseil') return 'Lire le conseil →'
                      if (type === 'outil' || type === 'checklist') return "Ouvrir l'outil →"
